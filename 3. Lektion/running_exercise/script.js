...
let list = [
	{ name: "Morten", topic: "Bajer", date: "15/02-2020", amount: "400", currency: "DDK" },
...
async function insertExpenses() {
	async function fetchExpenses() {
		try {
			const fetched_expenses = await fetch(`http://127.0.0.1:3000/expenses`).then((data) => data.json());
			return fetched_expenses;
		} catch (err) {
			console.log("FETCH ERROR! " + err);
		}
	}
	await fetchExpenses().then(add_rows).then(() => console.log("Fetch and insertion SUCCES!")).catch((err) => {
		console.log("Insertion of expenses FAILURE: " + err);
	});
}
...
async function insertFact() {
	...
	async function fetchFact() {
		try {
			const fetched_expenses = await fetch(`https://api.chucknorris.io/jokes/random`).then((data) => data.json());
			return fetched_expenses;
		} catch (err) {
			console.log("FETCH ERROR! " + err);
		}
	}
	await fetchFact().then((message) => {
		position.innerHTML = message.value;
	});
}

insertFact();

let knap = document.getElementById("new_fact");
